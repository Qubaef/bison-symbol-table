...
char single[MAX_STR_LEN + 1] = "single";
char list[MAX_STR_LEN + 1] = "list";
...
S_DECLARATION: DATA_TYPE VAR_LIST ';' { found("DECL", $2); };
...
STRUCTURE: KW_STRUCT OPT_TAG '{' FIELD_LIST '}' { found("STRUCT", ""); };
...
VAR_LIST: VAR { strcpy($$, single); }
    | VAR_LIST ',' VAR { strcpy($$, list); }
;
...
VAR: IDENT SUBSCRIPTS { found("VAR", $1); strcpy($$, $1); }
    | IDENT SUBSCRIPTS '=' EXPR { found("VAR", $1); strcpy($$, $1); }
    | IDENT SUBSCRIPTS '=' STRING_CONST { found("VAR", $1); strcpy($$, $1); }
;
...
BLOCK: INSTRUCTION { found("BLOCK", ""); }
    | '{' DECL_LIST INSTR_LIST '}' { found("BLOCK", ""); decScope(); }
;
